...
use control_plane::contracts::{
    AccountUsageLeaderboardItem, AccountUsageLeaderboardResponse, ApiKeyUsageLeaderboardItem,
...
struct FakeUsageRepo {
    account_leaderboard_items: Vec<AccountUsageLeaderboardItem>,
    account_leaderboard_calls: Arc<Mutex<Vec<AccountLeaderboardCall>>>,
}

impl FakeUsageRepo {
    fn account_leaderboard_calls(&self) -> Vec<AccountLeaderboardCall> {
        self.account_leaderboard_calls.lock().unwrap().clone()
    }
}
...
impl UsageQueryRepository for FakeUsageRepo {
    ...
    async fn query_account_leaderboard(
        ...
        self.account_leaderboard_calls
            .lock()
            .unwrap()
            .push(AccountLeaderboardCall {
                start_ts,
                end_ts,
                limit,
                account_id,
        ...
        Ok(self.account_leaderboard_items.clone())
    }
...
#[tokio::test]
async fn account_leaderboard_endpoint_returns_503_when_usage_repo_unavailable() {
    let app = build_app();
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/api/v1/usage/leaderboard/accounts?start_ts=1700000000&end_ts=1700003600")
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
#[tokio::test]
async fn account_leaderboard_endpoint_returns_400_when_start_ts_greater_than_end_ts() {
    let app = build_app();
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/api/v1/usage/leaderboard/accounts?start_ts=1700003600&end_ts=1700000000")
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
#[tokio::test]
async fn account_leaderboard_endpoint_uses_default_limit_when_missing() {
    ...
    let calls = usage_repo_handle.account_leaderboard_calls();
    assert_eq!(calls.len(), 1);
    assert_eq!(calls[0].limit, 20);
}

#[tokio::test]
async fn account_leaderboard_endpoint_returns_items_when_usage_repo_available() {
    ...
    let usage_repo = FakeUsageRepo {
        account_leaderboard_items: expected_items.clone(),
        ..Default::default()
    };
    let usage_repo_handle = usage_repo.clone();
    let store: Arc<dyn ControlPlaneStore> = Arc::new(InMemoryStore::default());
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method("GET")
                .uri(
                    "/api/v1/usage/leaderboard/accounts?start_ts=1700000000&end_ts=1700003600&limit=5",
                )
                .body(Body::empty())
                .unwrap(),
        )
    ...
    let body = to_bytes(response.into_body(), usize::MAX).await.unwrap();
    let payload: AccountUsageLeaderboardResponse = serde_json::from_slice(&body).unwrap();
...
#[tokio::test]
async fn account_leaderboard_endpoint_passes_account_id_filter_to_repo() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                ...
                .uri(
                    format!(
                        "/api/v1/usage/leaderboard/accounts?start_ts=1700000000&end_ts=1700003600&account_id={account_id}"
                    )
                    .as_str(),
                )
                .body(Body::empty())
    ...
    let calls = usage_repo_handle.account_leaderboard_calls();
    assert_eq!(calls.len(), 1);
    assert_eq!(calls[0].account_id, Some(account_id));
}
