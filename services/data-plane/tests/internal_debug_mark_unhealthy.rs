...
#[tokio::test]
async fn internal_debug_mark_unhealthy_route_returns_404_when_debug_routes_disabled() {
...
#[tokio::test]
async fn internal_debug_mark_unhealthy_route_requires_bearer_token() {
    let control_plane = MockServer::start().await;
    let accounts = test_upstream_accounts();
    let account_id = accounts[0].id;
    let app = build_test_app(
...
#[tokio::test]
async fn internal_debug_mark_unhealthy_route_marks_account_temporarily_unhealthy() {
    let control_plane = MockServer::start().await;
    ...
    assert_eq!(payload["temporarily_unhealthy"], true);
}

#[tokio::test]
async fn internal_debug_mark_unhealthy_route_returns_not_found_for_missing_account() {
    let control_plane = MockServer::start().await;
    let tenant_id = Uuid::new_v4();
    let api_key_id = Uuid::new_v4();
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(format!(
                    "/internal/v1/debug/accounts/{}/mark-unhealthy",
                    Uuid::new_v4()
                ))
                .header("authorization", "Bearer cp_valid_token")
                .body(Body::empty())
