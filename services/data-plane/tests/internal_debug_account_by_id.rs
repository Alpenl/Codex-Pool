...
#[tokio::test]
async fn internal_debug_account_by_id_route_returns_404_when_debug_routes_disabled() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method("GET")
                .uri(format!("/internal/v1/debug/accounts/{account_id}"))
...
#[tokio::test]
async fn internal_debug_account_by_id_route_requires_bearer_token() {
    let control_plane = MockServer::start().await;
    let accounts = test_upstream_accounts();
    let account_id = accounts[0].id;
...
#[tokio::test]
async fn internal_debug_account_by_id_route_returns_account_when_enabled_and_token_valid() {
    let control_plane = MockServer::start().await;
    ...
    assert_eq!(payload["label"], "chatgpt-account");
    assert_eq!(payload["mode"], "chat_gpt_session");
    assert_eq!(payload["enabled"], false);
    assert_eq!(payload["priority"], 80);
    assert_eq!(payload["base_url"], "https://chatgpt.com/backend-api/codex");
    assert_eq!(payload["chatgpt_account_id"], "acct_debug_1");
    assert_eq!(payload["temporarily_unhealthy"], false);
}

#[tokio::test]
async fn internal_debug_account_by_id_route_returns_not_found_for_missing_account() {
    let control_plane = MockServer::start().await;
    let tenant_id = Uuid::new_v4();
    let api_key_id = Uuid::new_v4();
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method("GET")
                .uri(format!("/internal/v1/debug/accounts/{}", Uuid::new_v4()))
                .header("authorization", "Bearer cp_valid_token")
                .body(Body::empty())
                .unwrap(),
        )
