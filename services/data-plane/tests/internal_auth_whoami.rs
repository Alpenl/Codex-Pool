...
#[tokio::test]
async fn internal_whoami_route_returns_404_when_debug_routes_disabled() {
...
#[tokio::test]
async fn internal_whoami_route_returns_principal_when_enabled_and_token_valid() {
    let control_plane = MockServer::start().await;
    ...
    let token = "cp_valid_token_123456";
    let response = app
        .oneshot(
            Request::builder()
                .method("GET")
    ...
    assert_eq!(payload["token_prefix"], "cp_valid");
}

#[tokio::test]
async fn internal_whoami_route_requires_bearer_token() {
    let control_plane = MockServer::start().await;
    let app = build_test_app(
        true,
        Some(format!("{}/internal/v1/auth/validate", control_plane.uri())),
...
#[tokio::test]
async fn internal_whoami_route_returns_structured_401_when_principal_missing() {
    let app = build_test_app(true, None, false, Vec::new()).await;
