...
fn test_upstream_accounts() -> Vec<UpstreamAccount> {
    vec![
        UpstreamAccount {
            id: Uuid::new_v4(),
            label: "debug-account-a".to_string(),
            mode: UpstreamMode::OpenAiApiKey,
            base_url: "https://api.openai.com/v1".to_string(),
            bearer_token: "tok-debug-a".to_string(),
        ...
        UpstreamAccount {
            id: Uuid::new_v4(),
            label: "debug-account-b".to_string(),
            mode: UpstreamMode::OpenAiApiKey,
            base_url: "https://api.openai.com/v1".to_string(),
            bearer_token: "tok-debug-b".to_string(),
            chatgpt_account_id: None,
            enabled: true,
            priority: 90,
            created_at: Utc::now(),
        ...
        UpstreamAccount {
            id: Uuid::new_v4(),
            label: "debug-account-c".to_string(),
            mode: UpstreamMode::ChatGptSession,
            base_url: "https://chatgpt.com/backend-api/codex".to_string(),
            bearer_token: "tok-debug-c".to_string(),
            chatgpt_account_id: Some("acct_debug_3".to_string()),
            enabled: true,
            priority: 80,
            created_at: Utc::now(),
        },
...
#[tokio::test]
async fn internal_debug_clear_unhealthy_route_returns_404_when_debug_routes_disabled() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/internal/v1/debug/accounts/clear-unhealthy")
...
#[tokio::test]
async fn internal_debug_clear_unhealthy_route_requires_bearer_token() {
    let control_plane = MockServer::start().await;
    let app = build_test_app(
        true,
        Some(format!("{}/internal/v1/auth/validate", control_plane.uri())),
...
#[tokio::test]
async fn internal_debug_clear_unhealthy_route_clears_penalties_and_returns_cleared_count() {
    let control_plane = MockServer::start().await;
    ...
    let unhealthy_ids = vec![accounts[0].id, accounts[1].id];
    let app = build_test_app(
        true,
        Some(format!("{}/internal/v1/auth/validate", control_plane.uri())),
        accounts,
    ...
    for account_id in &unhealthy_ids {
    ...
    assert_eq!(clear_payload["cleared"], 2);
    ...
    let accounts_body = accounts_response
        .into_body()
        .collect()
        .await
        .unwrap()
        .to_bytes();
    let accounts_payload: Value = serde_json::from_slice(&accounts_body).unwrap();
    let diagnostics = accounts_payload["accounts"].as_array().unwrap();

    for account_id in unhealthy_ids {
        let account = diagnostics
            .iter()
            .find(|entry| entry["id"] == account_id.to_string())
            .expect("account should exist");
        assert_eq!(account["temporarily_unhealthy"], false);
    }
}
